import { checkNewUser, fetchUserProject, getAuthor, getProjects_notUser } from "@/utils/mongo";
...
export default async function ProjectPage({ params }) {
    ...
    // if((session?.role==="HOD" || session?.role==="Department") && session?.deptID){
    //     const author=await getAuthor(auid);
    //     if(author?.dept===session?.deptID){
            
    //     }else{
    //         redirect('/')
    //     }
    // }else if(session?.role==="Author" && session?.scopusID){
    //     if(session?.scopusID===auid){
            ...
            {!(newUser) && <div className="col-span-12 self-stretch me-8">
                <CardContent>
                    <div style={{display:"flex",justifyContent: "space-between",alignItems:"center"}}>
                        ...
                        <h2>
                            {(session?.user?.email && ((session?.role==="Admin") || (session?.scopusID===auid && session?.role==="Author") || (session?.deptID===author?.dept && (session?.role==="HOD" || session?.role==="Department")))) && <Link href={`add`}>
                                    ...
                                    <div className="border-l-4 pl-3">
                                        ...
                                        >
                                            ...
                                            <div className="text-1xl" style={{display: "flex",alignContent:"center",justifyContent:"center",gap: "20px"}}>
                                                {(session?.user?.email && ((session?.role==="Admin") || (session?.scopusID===auid && session?.role==="Author") || (session?.deptID===author?.dept && (session?.role==="HOD" || session?.role==="Department")))) && <Link href={`edit/${project._id}`} style={{margin: "0px",padding: "0px"}}>
                                                    <svg
                                                ...
                                                {(session?.user?.email && ((session?.role==="Admin") || (session?.scopusID===auid && session?.role==="Author") || (session?.deptID===author?.dept && (session?.role==="HOD" || session?.role==="Department")))) && <DeleteButton id={project._id} auid={auid}/>}
                                            </div>
                                        </div>
                                        <div>
                                            <Link
